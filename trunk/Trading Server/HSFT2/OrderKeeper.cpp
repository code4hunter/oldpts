...
order_keeper_manager::KEEPERMANAGERPTR order_keeper_manager::_instance
	(new order_keeper_manager());
			...
			if (itf->Request(req, &ans, count) == 0) {
				if (count == 1) {
					...
					if (iter->leavesQty == 0) {
						...
						{
							double lastprice =
								(trdvol * price - oldtrdvol * oldprice)
								/ trdnum;
							...
							er.transactTime =
								Time().FormatString("hhnnss").ToInt();
...
void order_keeper::execute(const TSetAccount_Req *) {
	...
	TTrdItfBase *itf = new TTrdItf_HSFF(_setup_file.c_str(), _plogfunc, false);
	...
	try {
		while (_is_working) {
			...
			if (bneedInit) {
				...
				// 查询资金如果失败就重新连接
				if ((::GetTickCount() - oldtick1) >= 10000 || _is_panding_qry_money) {
					// qry money
					_is_panding_qry_money = false;
					request.QryMoney.ReqHead.Version = -100;
					request.QryMoney.ReqHead.FuncType = ftQryMoney;
					if (itf->Request(request, &ans, count) != 0) {
						ODS("->查询资金失败");
						bneedInit = true;
					}
					memcpy(&_money_ans,&(ans->QryMoney.MoneyInfo ),sizeof(TMoneyInfo));
					itf->FreeAnswer(ans);
				...
				// 不断取需要查撤单的委托
				// 获得未成交的委托数量，如果>0则查成交
				if ((::GetTickCount() - oldtick3) >= 1000) {
					// 每隔一秒钟查询成交流水
					request.QryCurTrd.ReqHead.FuncType = ftQryCurTrd;
					strcpy(request.QryCurTrd.OrderID, "-100");  //从柜台查
					request.QryCurTrd.MaxRecord = 300;
					std::string seq;
					database db;
					try {
						db.get_seq(Date().FormatString("yyyymmdd").ToInt(), seq,
							_account.Account);
					}
					catch (std::string &err) {
					...
					if (count > 0 && ans != NULL) {
						...
						for (int i = 0; i < count; i++) {
							...
							if (ti->Vol > 0) // 有成交
							{
								...
								try {
									db.add_trade(&er, ti->SeqNum,
										Time().FormatString("hhnnss").ToInt());
								}
								catch (std::string &err) {
									ODS("->add_trade失败,%s,%s", ti->SeqNum,
										err.c_str());
								}
							}
