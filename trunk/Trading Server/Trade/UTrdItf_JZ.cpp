...
{
  if(KDAPI_d) KDAPI_d();
  if(InterlockedDecrement(&link_count_jz)<=0)
  {
    if(hinstlib)
    {
...
int  TTrdItf_JZ::Connect(void)
{
  ...
  int i =( (KDAPI_c)(sa,FJZSet.CounterID,FJZSet.CounterPW));
...
/*
#ifdef _KD3X
    hinstlib = LoadLibrary("API_BH_3"); //"API_BH");
#else
    hinstlib = LoadLibrary("API_BH_2");
#endif
*/
    ...
    KDAPI_is = (KDAPI_IS)GetProcAddress(hinstlib,"KDAPI_InitSystem") ;
    KDAPI_c  = (KDAPI_C)GetProcAddress(hinstlib,"KDAPI_Connect") ;
    KDAPI_r  = (KDAPI_R)GetProcAddress(hinstlib,"KDAPI_Request") ;
    KDAPI_rf = (KDAPI_RF)GetProcAddress(hinstlib,"KDAPI_Request_fzdm") ;
    KDAPI_a  = (KDAPI_A)GetProcAddress(hinstlib,"KDAPI_Answer") ;
    ...
    KDAPI_d    = (KDAPI_DISC)GetProcAddress(hinstlib,"KDAPI_Disconnect") ;
    ...
    if (! (KDAPI_is)(10) )  { return -1 ; } /*本来为1000*/
  }
  ...
  if((retVal=Request(r,&a,num))>0) {
    if(num>0) {
      ...
      for(int i=0;i<num;i++)
      {
        ...
        memcpy((*FAnswer)[i].QryStocks.StockInfo.SecuDesc,a[i].gfcx.ZQMC,ZQMC_WIDTH);
