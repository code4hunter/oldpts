...
SDB_header head;
Sacct_pass pass;
extern int dedes(char *);
#if 0
void Sencrypt_init()
{
//		char my_key[] = "Hero from ShenZhen Named Wang Lin-Yun";
//		char my_key[] = "TBWLYSGW";
char my_key[] = "Hero fro";
desinit(0);
dessetkey(my_key);
return;
}
...
void Sencrypt_init()
{
//char my_key1[] = "Moonlight at Chelmsford";
char my_key[] = "WLYSGWTB";
desinit(0);
dessetkey(my_key);
endes(my_key);
desdone();
  ...
  return;
	...
	void Sendes(Schar *pass)
	{
	int i;
	
	  for (i=strlen(pass); i<9; i++)
	  pass[i] = 0;  // fill 0
	  endes(pass);
	  pass[8] = 0;   // terminate string
	  }
