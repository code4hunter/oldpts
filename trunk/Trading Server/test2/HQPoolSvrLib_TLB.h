...
// *********************************************************************//
template <typename T>
class IManager_EventsDispatcher : public IUnknown
{
public:
  HRESULT Fire_OnError(BSTR TopicName, long TopicID, BSTR ErrMessage)
  {
    _IManagerEventsDisp dispInvoker;
    ...
    if (cpsl.HasConnectionPoint()) {
      for (int i=0; i<cpsl.GetCount(); i++) {
        ...
        if (cpsl.GetSink(i, punk) && (dispInvoker.Bind(punk)==S_OK))
          dispInvoker.OnError(TopicName, TopicID, ErrMessage);
      }
    }
    return S_OK;
  }

  HRESULT Fire_OnEvent(BSTR TopicName, long TopicID, LPDISPATCH EventHead, LPDISPATCH EventData)
  {
    _IManagerEventsDisp dispInvoker;
    T* pClass = static_cast<T*>(this);
    TConnectionPointSinkList<T> cpsl(pClass->ConnectionPoint);
    if (cpsl.HasConnectionPoint()) {
      for (int i=0; i<cpsl.GetCount(); i++) {
        _di_IInterface punk;
        if (cpsl.GetSink(i, punk) && (dispInvoker.Bind(punk)==S_OK))
          dispInvoker.OnEvent(TopicName, TopicID, EventHead, EventData);
      }
    }
    return S_OK;
  }
