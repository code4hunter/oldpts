...
//---------------------------------------------------------------------------
TTrdItf_GTJA::TTrdItf_GTJA(const char *psetupfile,ADDLOG plogfunc)
...
//---------------------------------------------------------------------------
int TTrdItf_GTJA::QryDailyBill(int strdate,int enddate)
{
	AnsiString destfile = AnsiString("c:\\") +  FAccInfo.Capital + ".csv";
	...
	FSrcList->Clear();
	FDstList->Clear();
	const int nFieldCount = 6;
	FSrcList->Add("cleardate");
	//FSrcList->Add("bizdate");
	//FSrcList->Add("orderdate");
	//FSrcList->Add("ordertime");
	FSrcList->Add("digestid");
	FSrcList->Add("digestname");
	FSrcList->Add("fundid");
	FSrcList->Add("fundeffect");
	FSrcList->Add("fundbal");
	...
	while(nReturnCode == -1 || nReturnCode == 100 || nReturnCode == 0)
    {
        ...
        KCBPCLI_SetValue(FHandle, "strdate", IntToStr(strdate).c_str());
        KCBPCLI_SetValue(FHandle, "enddate", IntToStr(enddate).c_str());
        KCBPCLI_SetValue(FHandle, "fundid", FAccInfo.Capital);//资金帐户		int	N
        KCBPCLI_SetValue(FHandle, "printflag", "0");
        KCBPCLI_SetValue(FHandle, "qryflag", "1");
    ...
    if(FDstList->Count >= nFieldCount)
    {
        ...
        std::fstream filestr;
        filestr.open (destfile.c_str(), fstream::in | fstream::out);
        //set value;
        for(int i=0; i<nRows; i++)
        {
            for( int j=0;j<= nFieldCount;j++)
            {
                filestr << FDstList->Strings[i*nFieldCount+j].c_str() << ',';
            }
            filestr << std::endl;
        }
        filestr.close();
...
int TTrdItf_GTJA::QryStocks()    //查询帐号中所有的股份
{
  ...
  //QryDailyBill(FGTJASet.sdate,FGTJASet.edate);
...
int TTrdItf_GTJA::CheckRsOpen(void)
{
  ...
  if( strcmp(szTmpbuf, "0") != 0 )
  {
	...
	lstrcpyn(this->FLastError.Sender,"GTJAJZX",SEQ_LEN);
	if( sErrMsg.Pos("-990268060")>0  || sErrMsg.Pos("-990268140")>0 ) 
		this->FLastError.ErrorCode = ERR_UNDO_ALREADY_FILLED;
	else
...
bool TTrdItf_GTJA::GetConfig(void)
{
	...
	try
	{
		...
		FGTJASet.sdate = ini->ReadInteger(PLUGINNAME,"sdate",0);
        FGTJASet.edate = ini->ReadInteger(PLUGINNAME,"edate",0);
...
void TTrdItf_GTJA::SetConfig(void)
{
	...
	try
	{
        ...
        ini->WriteInteger(PLUGINNAME,"sdate",FGTJASet.sdate);
        ini->WriteInteger(PLUGINNAME,"edate",FGTJASet.edate);
		ini->UpdateFile();
	}
	__finally
	{
