...
{
  FrmMain->LogIt(AnsiString(log),ltODS);
}
...
void __fastcall TFrmMain::FormCloseQuery(TObject *Sender, bool &CanClose)
{
  if(!start->Enabled)
  {
    if(MessageBoxA(Handle,"服务器正在运行是否真的想要关闭服务器？","询问",
...
{
  UnicodeString aszPath=ChangeFileExt(Application->ExeName,L".ini");
  if(FileExists(aszPath))
	xInvokeExe(AnsiString(aszPath));
  else
...
void __fastcall TFrmMain::stopExecute(TObject *Sender)
{
  ...
  if(FServer->IsActive())
  {
    ...
    if(!FServer->IsActive())
    {
      Caption=Application->Title+L"-已关闭";
      LogIt("服务器已经关闭",ltMsg);
      start->Enabled=true;
...
void __fastcall TFrmMain::startExecute(TObject *Sender)
{
  ...
  if (false==FServer->IsActive())
  {
    ...
    if(FServer->IsActive())
    {
      ...
      LogIt(AnsiString("服务器已经启动 Listening at ") + FServer->get_local_ip() + ":" +
...
void TFrmMain::LogIt(AnsiString Text,TLogType lt)
{
  AnsiString str;
  ...
  {
    CS->Enter();
    FLog->Add(str + AnsiString(DateTimeToStr(Now())) + "] " + Text);
  }
  __finally
  {
    CS->Leave();
...
{
  sb->Panels->Items[0]->Text=L"客户数: " + IntToStr((int)FServer->m_client_num);
  sb->Panels->Items[1]->Text=L"请求数: " + IntToStr((int)g_req_num);
}
...
void TFrmMain::AddOrder(void)
{
  ...
  {
    AnsiString filename = g_stp->OrderFilePath;
