...
void getAccount(UnicodeString line,AnsiString &svr,AnsiString &acc,AnsiString &pwd)
{
	char buf[255];
	char *pPass = NULL;
	char *pacc = NULL;
...
//
	for(int i=0;i<Memo2->Lines->Count;i++)
	{
		...
		{
			AnsiString acc,code,mkt,num,price,ordtype;
			getOrder(AnsiString(line),acc,code,mkt,num,price,ordtype);
			if( cbonlylist->Checked )
			{
